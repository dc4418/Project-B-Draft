...
var w = 640,
    h = 480;
var mouthOpenThreshold = 100; // Adjust this threshold as needed
...
function draw() {
    ...
    if (positions.length > 0) {
        ...
        if (mouthOpen <= mouthOpenThreshold) {
            // Draw black background if mouth is open
            background(0);
            textSize(32);
...
function drawCigarette(x, y) {
    ...
    rectMode(CENTER);
    rect(x/2, y + whiteLength * 0.5, cigaretteWidth, whiteLength*2, 5);

    // Draw the yellowish part of the cigarette
    fill(40, 100, 100); // Yellowish color for the cigarette filler
    rect(x/2, y - whiteLength * 0.5, cigaretteWidth, whiteLength, 5);
}